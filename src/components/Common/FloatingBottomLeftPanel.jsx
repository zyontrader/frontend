...
const FloatingBottomLeftPanel = ({ header = "Actions" }) => {
  ...
  return (
    <div
      className={`${
        expanded ? "h-[50vh]" : "h-6"
      } w-96 fixed left-0 bottom-0 z-50 bg-neutral-900 border border-neutral-700 rounded-tr-xl shadow-lg transition-all duration-200 overflow-hidden flex flex-col`}
    >
      <div
      ...
      <div
        className={`overflow-y-auto opacity-100 ${
          expanded ? "flex-1 h-auto" : "h-0"
        }`}
      >
